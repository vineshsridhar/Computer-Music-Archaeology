# m2p2 -- Music file to pic  -- 2 lines, 1 with inverted stems
...
/^#/	{
	if ($1 == "#TITLE") {
		...
		nsym[t * nv + 1] = sym "sbarline(bb,bt); n=n+1.9*w"
		nsw[t * nv + 1] = 1.9
		while (numaccs > 0)
			acc[acclist[--numaccs]] = ""
		t++
...
END	{
	...
	for (time = 0; time < t; time++) {
		if (x > 55 * scale || (x > 48 * scale && psw == 1.9)) {
			print "eseq(blet, \"3\")"
			if (psw == 1.9)
				print "n=n-1.9*w"
			print staves
		...
		for (voice = 1; voice <= 2; voice++) {
		    i = time * nv + voice
		    q = nsym[i]
		    if (q != "") {
			    print "stem =", 2 * voice - 3
			    print q
		    }
		    psw = nsw[i]
		    x = x + psw
