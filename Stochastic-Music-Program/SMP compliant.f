...
C
	DIMENSION Q(12),S(12),E(12,12),PN(12,50),SPN(12,50),NT(12),HAMIN(12,50),HAMAX(12,50),HBMIN(12,50),HBMAX(12,50),GN(12,50),H(12,50),TETA(256),VIGL(3),MODI(7),Z1(8),Z2(8),ALFA(3),AMAX(12)
C
C
C
	I=1
...
30	FORMAT(6(F3.2,F9.8)/F3.2,F9.8,E6.2,F9.8)
	PRINT 40,TETA,Z1,Z2
40	FORMAT(*1  THE TETA TABLE = *,/,21(12F10.6,/),4F10.6,/////,* THE Z1 TABLE = *,/,7F6.2,E12.3,///,* THE Z2 TABLE = *,/,8F14.8,/,1H1)
	READ 50,DELTA,V3,A10,A20,A17,A30,A35,BF,SQPI,EPSI,VITLIM,ALEA,LIM
...
60	FORMAT(5I3,2I2,2F6.0,I2I2)
	PRINT 70,DELTA,V3,A10,A20,A17,A30,A35,BF,SQPI,EPSI,VITLIM,ALEA,LIM,KT1,KT2,KW,KNL,KTR,KTE,KR1,GTNA,GTNS,((I,NT(I)),I=1,KTR)
70	FORMAT(*1DELTA = *,F4.0,/,* V3 = *,F6.3,/,* A10 = *,F4.1,/,* A20 = *,F4.1,/,* A17 = *,F4.1,/,* A30 = *,F4.1,/,* A35 = *,F4.1,/,* BF = *,F3.0,/,* SQP1 =*,F11.8,/,* EPSI =*,F12.8,/,* VITLIM = *,F5.2,/,* ALEA =*,F12.8,/,* ALIM = *,F6.2,/,* KT1 = *,I3,/,* KT2 = *,I3,/,* KW = *,I3,/,* KNL = *,I3,/,* KTR = *,I3,/,* KTE = *,I2,/,* KR1 = *,I2,/,* GTNA = *,F7.0,/,* GTNS = *,F7.0,/,I2(* IN CLASS *,I2,*, THERE ARE *,I2,* INSTRUMENTS.*,/))
...
C	EACH RUN OF THIS PROGRAM WILL PRODUCE DIFFERENT OUTPUT DATA.
	...
	Y=0.0
	KTS=NT(I)
	READ 100,(HAMIN(I,J),HAMAX(I,J),HBMIN(I,J),HBMAX(I,J),GN(I,J),PN(I,J),J=1,KTS)
100	FORMAT (5(5F2.0,F3.3))
	PRINT 110,I,(J,HAMIN(I,J),HAMAX(I,J),HBMIN(I,J),HBMAX(I,J),GN(I,J),PN(I,J),J=1,KTS)
110	FORMAT(////,* IN CLASS NUMBER *,I2,(/,* FOR INSTRUMENT NO. *,I2,* HAMIN = *,F3.0,*,HAMAX = *,F3.0,*,HBMIN = *,F3.0,*,HBMAX = *, F3.0,*,GN = *,F3.0,*, AND PN = *,F6.3))
	DO 120 J=1,KTS
	Y=Y+PN(I,J)
120	SPN(I,J)=Y
...
160	FORMAT(//////,* CLASS NUMBER *,I2,/,(* IN DENSITY LEVEL *,I2,* HAS A PROBABILITY OF *,F6.2,/))
	DO 180 J=1,KTE
	Y=0.0
	DO 170 I=1,KTR
170	Y=Y+E(I,J)
...
860	FORMAT(*1  JW=*,I3,4X,*A=*,F8.2,4X,*NA=*,I6,4X,*Q(I)=*,I2(F4.2,*/*),//)
	PRINT 870
870	FORMAT(6X,*N*,8X,*START*,5X,*CLASS*,4X,*INSTRM*,4X,*PITCH*,6X,*GLISS1*,4X,*GLISS2*,4X,*GLISS3*,8X,*DURATION*,5X,*DYNAM*)
	NLINE=1
